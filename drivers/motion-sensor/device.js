...
module.exports = class MotionSensorDevice extends YoLinkDevice
{
   ...
   */
	...
	{
		this.homey.app.updateLog('MotionSensorDevice settings were changed');
	}
   ...
   */
	...
	async updateState()
	{
		...
		this.setCapabilityValue('alarm_motion', state.data.state.state === 'alert').catch(this.error);
		this.setCapabilityValue('measure_temperature', parseFloat(state.data.state.devTemperature)).catch(this.error);
	...
	async processMQTTMessage(mqttMessage)
	{
		...
		this.setCapabilityValue('alarm_motion', mqttData.state === 'alert').catch(this.error);
		if (mqttData.devTemperature)
		{
			this.setCapabilityValue('measure_temperature', parseFloat(mqttData.devTemperature)).catch(this.error);
		}
